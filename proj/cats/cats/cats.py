...
def accuracy(typed, reference):
    ...
    if len_t == 0 and len_r!=0 or len_t != 0 and len_r == 0 :
        return 0.0
    if len_t == 0 and len_r == 0 :
        return 100.0 
    total = 0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    for type_w in word_list :
        if type_w == typed_word :
            return typed_word
    ans = {}
    count = 0 
    min_Differ = diff_function(typed_word, word_list[0], limit) 
    ans = {"0": word_list[0]}
    for i in word_list :
        count = diff_function(typed_word, i, limit)
        if count <= limit:
            if count < min_Differ:
                min_Differ = count
                ans["0"] = i
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    res = []
    for tim in times_per_player :
        res.append([tim[i] - tim[i-1] for i in range(1, len(tim))])
