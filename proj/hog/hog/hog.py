...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if dice :
            ...
            if who:
                score1 += take_turn(strategy1(score1, score0), score1, score0, dice, goal)
                score1 += hefty_hogs(score1 ,score0)
                score1 += hog_pile(score1, score0)
                leader, msg = say(score0, score1, leader)
                if msg:
                    print(msg)
                ...
                score0 += take_turn(strategy0(score0, score1), score0, score1, dice, goal)
                score0 += hefty_hogs(score0, score1)
                score0 += hog_pile(score0, score1)
                leader, msg = say(score0, score1, leader)
                if msg:
                    print(msg)            
    ...
    if res == 0 :
        ...
        if res > 0 :
            ...
            if last_leader != current_leader :
                return current_leader,  f'Player {current_leader} takes the lead by {res}'
            else :
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    total = hefty_hogs(score, opponent_score)
    if total >= threshold :
        return 0
    else: 
        return num_rolls
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if hog_pile(score+ hefty_hogs(score, opponent_score), opponent_score) or hefty_hogs_strategy(score, opponent_score, threshold, num_rolls) == 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
